...
class QuatWeightConLinear(nn.Module):
    def __init__(self, in_size, out_size, bias=None, device=None, dtype=None):
        super().__init__()
        factory_kwargs = {'device': device, 'dtype': dtype}
        assert in_size % 4 == 0, "quaternion linear input size must be divisible by 4"
        assert out_size % 4 == 0, "quaternion linear output size must be divisible by 4"
        weight_size_in = in_size // 4
        weight_size_out = out_size // 4

        self.w_r = nn.Parameter(torch.rand((weight_size_in, weight_size_out), **factory_kwargs))
        self.w_i = nn.Parameter(torch.rand((weight_size_in, weight_size_out), **factory_kwargs))
        self.w_j = nn.Parameter(torch.rand((weight_size_in, weight_size_out), **factory_kwargs))
        self.w_k = nn.Parameter(torch.rand((weight_size_in, weight_size_out), **factory_kwargs))
        

        torch.nn.init.normal_(self.w_r, mean=0.0, std=0.01)
        torch.nn.init.normal_(self.w_i, mean=0.0, std=0.01)
        torch.nn.init.normal_(self.w_j, mean=0.0, std=0.01)
        torch.nn.init.normal_(self.w_k, mean=0.0, std=0.01)

        if bias:
            self.bias = nn.Parameter(torch.zeros(out_size, **factory_kwargs))
        else:
            self.register_parameter('bias', None)

    def forward(self, x):

        cat_kernels_4_r = torch.cat([self.w_r, -self.w_i, -self.w_j, -self.w_k], dim=0)
        cat_kernels_4_i = torch.cat([self.w_i,  self.w_r, -self.w_k, self.w_j], dim=0)
        cat_kernels_4_j = torch.cat([self.w_j,  self.w_k, self.w_r, -self.w_i], dim=0)
        cat_kernels_4_k = torch.cat([self.w_k,  -self.w_j, self.w_i, self.w_r], dim=0)
        cat_kernels_4_quaternion = torch.cat([cat_kernels_4_r, cat_kernels_4_i, cat_kernels_4_j, cat_kernels_4_k], dim=1)
        mat_mul = torch.matmul(x, cat_kernels_4_quaternion)


        return mat_mul + (self.bias if self.bias else 0)
...
class RWKV_TimeMix_x051a(nn.Module):
    ...
    def __init__(self, config, layer_id):
        ...
        self.receptance = QuatWeightConLinear(config.n_embd, config.n_embd, bias=config.bias)
        self.key = QuatWeightConLinear(config.n_embd, config.n_embd, bias=config.bias)
        self.value = QuatWeightConLinear(config.n_embd, config.n_embd, bias=config.bias)
        self.gate = QuatWeightConLinear(config.n_embd, config.n_embd, bias=config.bias)

        self.output = QuatWeightConLinear(config.n_embd, config.n_embd, bias=config.bias)
...
class RWKV_ChannelMix_x051a(nn.Module):
    ...
    def __init__(self, config, layer_id):
        ...
        self.key = QuatWeightConLinear(config.n_embd, 3 * config.n_embd, bias=config.bias)
        self.value = QuatWeightConLinear(3 * config.n_embd, config.n_embd, bias=config.bias)
        self.receptance = QuatWeightConLinear(config.n_embd, config.n_embd, bias=config.bias)
        self.dropout = nn.Dropout(config.dropout)
